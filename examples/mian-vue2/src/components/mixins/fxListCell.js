	...
	methods: {
		...
		onTableRowValidate (showValidate, clearValidate) {
			return (item) => {
				...
				if (this.$fxUtils.isDef(this.subRowIndex)) {
					if (this.rowIndex === item.index && this.subRowIndex === item.subIndex && this.columnProp === item.columnProp) {
		...
		cellSelected (event) {
			if (this.isCollectionCell) return
			const cellEvent = {
