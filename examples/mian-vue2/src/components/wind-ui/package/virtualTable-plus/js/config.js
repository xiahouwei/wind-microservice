...
const cellRender = {
	...
	expand: {
		...
		renderCell (h, column, columnIndex, rowItem, rowIndex, store) {
			...
			const isPrint = rowItem.groupType
			const arrowClass = isPrint ? '' : isExpanded ? 'el-icon-arrow-down' : 'el-icon-arrow-right'
			...
			return !rowItem.additionFlag && rowItem.dosingDetails.length > 0 ? <div class={ classes } on-click={callback}>
				<i class={['el-icon', arrowClass, 'expand-icon']} style={{ display: 'inline-block', position: 'absolute', left: '8px', top: 'calc(50% - 7px)' }}></i><span>{cell}</span>
			</div> : <div class={ classes } on-click={callback}>
				<span>{cell}</span>
			</div>
		}
	},
