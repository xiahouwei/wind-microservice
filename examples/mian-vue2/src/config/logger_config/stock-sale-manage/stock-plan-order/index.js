...
export default {
	...
	actions: [{
		...
		id: 'mergeBill',
		label: '合并',
		messageHandler ({ oldBillCodes, newOrderData }) {
			return `系统单号:${oldBillCodes.join(',')};合并为系统单号:${newOrderData ? this.createCodeSpan(newOrderData, orderType) : '/'}`
		}
	}, {
		id: 'autoMergeBill',
		label: '自动合并',
		messageHandler ({ mergeData }) {
			if (Array.isArray(mergeData) && mergeData.length) {
				return mergeData.map(item => {
					return `${item.mergeMemo}【${this.createCodeSpan(item, orderType)}】`
				}).join(';')
			} else {
				return '自动合并失败'
			}
		...
		id: 'settingMulityBill',
		label: '批量设置',
		messageHandler ({ orderData, settingInfo, success, failed }) {
			let message = ''
			orderData.forEach(item => {
				if (success.length > 0 && success.includes(item.id)) {
					message += `系统单号:${this.createCodeSpan(item, orderType)},修改信息为:${settingInfo.fieldName}由“${item.operateManName}”改为“${settingInfo.operateManName}”,操作状态:成功;`
				} else if (failed.length > 0 && failed.includes(item.id)) {
					message += `系统单号:${this.createCodeSpan(item, orderType)},修改信息为:${settingInfo.fieldName}由“${item.operateManName}”改为“${settingInfo.operateManName}”,操作状态:失败;`
				}
			})
			return message
		}
