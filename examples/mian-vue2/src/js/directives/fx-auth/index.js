...
const getVShowDirective = function (vnode) {
	const showDirective = vnode.data.directives.find(item => item.rawName === 'v-show')
	if (showDirective) {
		return showDirective.value
	}
}
const verifyAuth = function (el, binding, vnode) {
	if (getVShowDirective(vnode) === false) {
		return false
	}
