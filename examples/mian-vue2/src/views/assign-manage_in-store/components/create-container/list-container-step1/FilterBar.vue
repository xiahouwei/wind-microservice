<template>
	<fx-list-filter label-width="90px" @on-enter="onSearchClick">
		<template slot="row-1">
			...
			<el-form-item label="入库分派状态">
				<w-select-mulity
					v-model="inAssignStatus"
...
export default {
	...
	data () {
		return {
			...
			inAssignStateList: this.$fxStateMiddleware.getStateConfig('inAssignState').slice(0, -1),
			...
			billCreateTypeList: this.$fxTypeMiddleware.getCommonTypeList('saleBillType'),
			organIdList: [],
			itemList: [],
			billBizTypeList: [],
			purchasePlanList: [],
	...
	methods: {
		...
		onResetClick ({ businessDate, inAssignStatus, clearDate, organId }) {
