	...
	<div class="organ-manage-container">
		...
		>
			<div slot="btn-tools-left" class="tools-left">
				<el-button v-fx-auth:stock-sale-manage_assign-manage_in-store.lock-summary class="order-btn" type="primary" plain @click='onLockAssign'>锁定并汇总</el-button>
				<el-button v-fx-auth:stock-sale-manage_assign-manage_in-store.unlock-instore class="order-btn" type="danger" plain @click='onUnlock'>解除锁定</el-button>
				<el-button v-fx-auth:stock-sale-manage_assign-manage_in-store.giveup-instore class="order-btn" type="primary" plain @click='onGiveupAll'>整单放弃</el-button>
				<el-button v-fx-auth:stock-sale-manage_assign-manage_in-store.show-giveup class="order-btn" type="danger" plain @click='showGiveup'>查看放弃</el-button>
			</div>
		</fx-list-virtual-table>
		...
		onGiveupAll () {
			this.$refs.listTable.getSelectedRows().then(res => {
				this.$fxConfirm({
					...
					const logger = this.$fxLogger.createInfo('stock-sale-manage.assign-manage.in-assign.giveUpAll', { data: res })
		...
		onUnlock () {
			this.$refs.listTable.getSelectedRows().then(res => {
				this.$fxConfirm({
					...
					const logger = this.$fxLogger.createInfo('stock-sale-manage.assign-manage.in-assign.unlock', { data: res })
