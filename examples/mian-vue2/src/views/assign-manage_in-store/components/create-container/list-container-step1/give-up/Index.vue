		...
		<div class="organ-manage-container">
			...
			>
				<div slot="btn-tools-left" class="tools-left">
					<el-button v-fx-auth:stock-sale-manage_assign-manage_in-store.recover-instore class="order-btn" type="primary" plain @click='onRecover'>恢复</el-button>
				</div>
			</fx-details-table>
		</div>
	</w-dialog>
		...
		return {
			...
			tableValiDate: [{
				...
				rule: (row, success, error) => {
					if (row.thisAmount > row.inGiveUpAmount) {
		...
		onRecover () {
			return this.$refs.detailsTable.getSelectRows().then(res => {
				...
				for (const item of res) {
					if (item.thisAmount > item.inGiveUpAmount) {
				...
				if (flag) {
					this.$fxConfirm({
						...
						const logger = this.$fxLogger.createInfo('stock-sale-manage.assign-manage.in-assign.recover', { data: res })
