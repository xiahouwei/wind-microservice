...
export default {
	...
	methods: {
		...
		onSaveClick () {
			...
			const logger = this.$fxLogger.createInfo('basic-archives.employee-manage.copyOrgan', { selectRows: this.$refs.transfer.getSelectRows(), name: this.itemDetails.name, code: this.itemDetails.code })
			return this.$fxApi('employee.copyConnectWithEmployee', this.itemDetails.id)({ data: this.listSelectIds, logger }).then(res => {
