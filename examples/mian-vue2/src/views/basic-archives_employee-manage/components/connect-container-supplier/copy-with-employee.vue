	...
	<w-dialog v-model="dialogVisible" width="1200px" title="选择员工" heightFullscreen @close="close">
		<div class="create-organ">
			...
			<div class="list-container">
				<div class="list-container__title">注：选择某员工后，当前员工可管理供货商的关系会与被选择的员工保持一致</div>
	...
	data () {
		return {
			...
			tableColumn: [
				...
				{ prop: 'organName', label: '直属机构', required: true },
				{
					prop: 'organType',
					label: '直属机构类型',
					required: true,
				...
				{
					prop: 'manageAllSupplier',
					label: '可管理供货商设置',
					required: true,
					formatter: (row) => {
						return this.$fxTypeMiddleware.getCommonTypeItemById('manageAllSupplierType')(row.manageAllSupplier).label
					}
				}
			],
			tableData: [],
	...
	methods: {
		...
		onSaveClick () {
			return this.$refs.listTable.getSingleSelectedRow().then(res => {
				const logger = this.$fxLogger.createInfo('basic-archives.employee-manage.copyWithEmployee_supplier', { copyName: res.name, name: this.itemDetails.name, code: this.itemDetails.code })
				return this.$fxApi('employee.copyConnectSupplierEmployee2Employee', res.id, this.itemDetails.id)({ logger }).then(() => {
					this.dialogVisible = false
					this.$emit('do-close', res.manageAllSupplier)
		...
		getConnectList () {
			return this.$fxApi('employee.getEmployeeExcludeIdSupplier', this.itemDetails.id).then(res => {
