<template>
	<w-dialog v-model="dialogVisible" width="1200px" title="关联品项" heightFullscreen @close="close">
		...
		<template slot="footer">
			...
			<w-link v-fx-auth:basic-archives_organ-manage.edit-organ icon="copy-single" @click="onConnectItemWithStore">与其他{{organTypeLabel}}一致</w-link>
			<w-link v-fx-auth:basic-archives_organ-manage.edit-organ icon="copy-mulity" @click="onCopyConnect">复制关系</w-link>
			<el-button @click="onCancelClick">取消</el-button>
			<el-button v-fx-auth:basic-archives_organ-manage.edit-organ type="primary" @click="onSaveClick">保存</el-button>
		</template>
...
export default {
	...
	computed: {
		...
		organTypeLabel () {
			return this.$fxTypeMiddleware.getCommonTypeItemById('organType')(this.itemDetails.organType).label
		}
	},
	methods: {
		open (item) {
		...
		onSaveClick () {
			...
			return new Promise(resolve => {
				...
				const logger = this.$fxLogger.createInfo('basic-archives.organ-manage.connectItem', this.createLoggerParams(this.fxDataVerification.getOriginData(), selectRows))
				this.$fxApi('organ.saveConnectItem', this.itemDetails.organType, this.itemDetails.id)({ data: this.listSelectIds, logger }).then(() => {
					this.fxDataVerification.resetData(selectRows)
					resolve()
				})
			})
		...
		onExport () {
			const logger = this.$fxLogger.createInfo('basic-archives.organ-manage.exportConnectItem')
			return this.$fxApi('organ.exportConnectItem', this.itemDetails.id)({ logger }).then(res => {
