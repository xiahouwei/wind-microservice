			...
			<fx-details-form-row>
				<el-form-item label="备注：">
					<w-input
						...
						v-fx-tab:loop.basic-archives_organ-manage_details.appendInfo
						:maxlength="400"
						class="form-item-input"
						placeholder="请输入备注"
						:disabled="disabled"
...
export default {
	...
	methods: {
		onFileChange (list) {
			const _appendixList = this.organDetails.appendixList
			this.organDetails.appendixList = list.map(item => {
				const file = _appendixList.find(_item => {
					return _item.appendixUrl === item
				}) || {}
				return {
					id: file.id || null,
					appendixUrl: item
