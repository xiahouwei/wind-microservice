...
export default {
	...
	methods: {
		...
		onSaveClick () {
			...
			return this.$fxApi('role.saveConnectRoles', this.itemDetails.id)({ data: this.listSelectIds, logger }).then(res => {
				this.fxDataVerification.resetData(this.listSelectIds)
				this.rowDataVerification.resetData(selectRows)
		...
		refreshConnectList () {
			return this.$fxApi('role.getConnectRolesById', this.itemDetails.id).then(res => {
				this.tableData = res
