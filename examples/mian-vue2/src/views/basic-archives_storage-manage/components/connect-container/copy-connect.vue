...
export default {
	...
	methods: {
		...
		onSaveClick () {
			...
			const logger = this.$fxLogger.createInfo('basic-archives.organ-manage.copyConnect', { selectRows: this.$refs.transfer.getSelectRows(), name: this.itemDetails.name, type: this.organTypeLabel })
			return this.$fxApi('organ.copyConnectWithSection', this.itemDetails.organType, this.itemDetails.id)({ data: this.listSelectIds, logger }).then(res => {
