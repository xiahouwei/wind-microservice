...
export default {
	...
	methods: {
		...
		onSaveClick () {
			...
			const logger = this.$fxLogger.createInfo('basic-archives.supplier-manage.copyConnect', { selectRows: this.$refs.transfer.getSelectRows(), name: this.itemDetails.name, type: this.organTypeLabel })
			if (this.supplierType === 'supplier') {
				return this.$fxApi('supplier.copySupplierItemWithSection', this.itemDetails.id)({ data: this.listSelectIds, logger }).then(res => {
					return Promise.resolve()
				})
			} else {
				return this.$fxApi('supplier.copyConnectWithSection', this.itemDetails.organType, this.itemDetails.id)({ data: this.listSelectIds, logger }).then(res => {
					return Promise.resolve()
				})
			}
		},
