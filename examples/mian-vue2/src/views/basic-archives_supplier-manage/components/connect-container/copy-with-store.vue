	...
	<w-dialog v-model="dialogVisible" width="1200px" title="选择品项" heightFullscreen @close="close">
		<div class="create-organ">
			...
			<div class="list-container">
				<div class="list-container__title">注：选择某供货商后，当前供货商与品项的关联关系会与被选择的供货商保持一致</div>
...
export default {
	...
	methods: {
		open (item) {
			...
			this.supplierType = this.$fxTypeMiddleware.getCommonTypeItemById('supplierType')(this.itemDetails.organType).type
		...
		onSaveClick () {
			return this.$refs.listTable.getSingleSelectedRow().then(res => {
				const logger = this.$fxLogger.createInfo('basic-archives.supplier-manage.copyWithStore', { copyName: res.name, name: this.itemDetails.name, type: this.organTypeLabel })
				if (this.supplierType === 'supplier') {
					return this.$fxApi('supplier.copyOtherSupplierItem', res.id, this.itemDetails.id)({ logger }).then(() => {
						this.dialogVisible = false
						this.$emit('do-close', res.relAllGoods)
						return Promise.resolve()
					})
				} else {
					return this.$fxApi('supplier.copyConnectOrgan2Organ', this.itemDetails.organType, res.id, this.itemDetails.id)({ logger }).then(() => {
						this.dialogVisible = false
						this.$emit('do-close', res.relAllGoods)
						return Promise.resolve()
					})
		...
		getConnectList () {
			if (this.supplierType === 'supplier') {
				return this.$fxApi('supplier.getSupplierItem', this.itemDetails.id).then(res => {
					this.defaultTableSelect = []
					this.tableData = res
					return Promise.resolve()
				})
			} else {
				return this.$fxApi('supplier.getSupplierExcludeId', this.itemDetails.organType, this.itemDetails.id).then(res => {
					this.defaultTableSelect = []
					this.tableData = res
					return Promise.resolve()
				})
