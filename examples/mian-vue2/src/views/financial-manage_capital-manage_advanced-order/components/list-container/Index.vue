		...
		>
			<div slot="btn-tools-left" class="tools-left">
				<fx-list-btn-tools :recycle="false">
					...
					<!-- <fx-button class="order-btn" v-fx-auth:store-manage_in-store-order.recheck-instore type="danger" plain @click="onPrintClick">打印</fx-button> -->
				</fx-list-btn-tools>
			</div>
...
export const BILL_TYPE = 'AdvancedOrder'
export default {
	name: 'listContainer',
	components: {
		filterBar
	...
	methods: {
		...
		getSearchParams () {
			...
			return {
				...
				...this.$fxCommon.getSearchParams('advancedOrderList')(params),
				sumMoney: sumCols
			}
		},
		getListData () {
		...
		onDeleteHandler (selectRows) {
			...
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.delete', { orderData: selectRows })
		...
		onSubmitHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.submit', { orderData: selectRows })
		...
		onReturnHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.return', { orderData: selectRows })
		...
		onCheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.check', { orderData: selectRows })
		...
		onUncheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.uncheck', { orderData: selectRows })
		...
		onRecheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.recheck', { orderData: selectRows })
		...
		onUnrecheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.advanced-order.unrecheck', { orderData: selectRows })
