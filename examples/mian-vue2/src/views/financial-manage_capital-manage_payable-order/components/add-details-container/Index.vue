...
export default {
	...
	methods: {
		...
		getListDataHandler () {
			return this.$fxApi(`${apiName}.getFinancialInStorePage`)({ data: this.getSearchParams() })
		},
		onCreateClick () {
			this.$refs.listTable.getSelectRows().then(selectRows => {
				const ids = selectRows.map(item => item.id)
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.addDetails', { orderData: this.organDetails, inStoreOrders: selectRows })
