<template>
	<w-collapse v-model="collapseVisible" contentHeight="480px" :fullScreen="isFullScreen" disabled :showArrow="false" title="明细信息">
		<fx-details-table
			ref="detailsTable"
			v-fx-tab:loop.financial-manage_payable-order_details.detailsInfo
			:disabled="disabled"
...
export default {
	...
	props: {
		...
		disabled: Boolean,
		fpSaveMain: Function,
		isRedPayableOrder: Boolean
	},
	data () {
		return {
			collapseVisible: true,
			tableColumn: [
				{ prop: 'inBillCode', label: '入库单号', width: '150px' },
			...
			fxCalculation: this.$fxCalculation('payableOrder'),
	...
	methods: {
		cellClassName (row, rowIndex, column) {
			if (['amount', 'price', 'taxPrice', 'taxRate', 'money', 'taxRateMoney', 'taxMoney'].includes(column.prop)) {
		...
		taxRateRender (h, { row, $index }) {
			...
			return <fx-list-cell-select
				ref={`taxRate-${$index}`}
				vModel={row.taxRate}
				isEdit={row.isEdit && !this.isRedPayableOrder}
				row={row}
				rowIndex={$index}
				columnProp='taxRate'
		...
		taxRateMoneyRender (h, { row, $index }) {
			...
			return <div class='flex-box'>
				<div class='minus-box'> {this.isRedPayableOrder && row.isEdit ? '-' : ''} </div>
				<fx-list-cell-input-number
		...
		onSaveDetailsHandler (params, origin) {
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.updateDetails', this.createLoggerParams(params, origin))
