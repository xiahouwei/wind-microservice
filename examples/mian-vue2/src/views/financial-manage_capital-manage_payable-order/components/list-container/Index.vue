		...
		>
			<div slot="btn-tools-left" class="tools-left">
				<fx-list-btn-tools :recycle="false">
					...
					<fx-button class="order-btn" v-fx-auth:store-manage_in-store-order.recheck-instore type="primary" plain @click="onPushDownClick">下推</fx-button>
				</fx-list-btn-tools>
			</div>
...
export const BILL_TYPE = 'PayableOrder'
export default {
	name: 'listContainer',
	components: {
		filterBar
		...
		return {
			tableColumnMap: {
				...
				sumTaxMoney: {
					...
					required: true,
					summary: true,
					fxRender: this.sumTaxMoneyRender,
	...
	methods: {
		...
		onDeleteHandler (selectRows) {
			...
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.delete', { orderData: selectRows })
		...
		onSubmitHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.submit', { orderData: selectRows })
		...
		onReturnHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.return', { orderData: selectRows })
		...
		onCheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.check', { orderData: selectRows })
		...
		onUncheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.uncheck', { orderData: selectRows })
		...
		onRecheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.recheck', { orderData: selectRows })
		...
		onUnrecheckHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.unrecheck', { orderData: selectRows })
		...
		onPushDownHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payable-order.pushdown', { orderData: selectRows })
		...
		sumTaxMoneyRender (h, { row }) {
			return <span class={['fx-list-table__cell-span', this.billClass(row)]}>{this.$fxCommon.toFixedAutoZeroFill(row.sumTaxMoney, this.sysMoneyPointSize)}</span>
		},
