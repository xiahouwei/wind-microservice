<template>
	<fx-list-filter @on-enter="onSearchClick">
		<template slot="row-1">
			...
			<el-form-item prop="dateRang" label="业务日期">
				<w-date-picker
...
export default {
	...
	computed: {
		...
		departmentName () {
			return this.organDetails.department.name || ''
		}
	},
	methods: {
		onSearchClick () {
		...
		getFilterParams () {
			...
			this.filter.departmentId = [this.organDetails.department.id || '']
			this.filter.sysBillCode = this.filter.userBillCode
			this.filter.id = this.connectBillDetails.map(item => item.payableId)
			return this.filter
		}
	}
}
