<template>
	<w-collapse v-model="collapseVisible" contentHeight="320px" disabled :showArrow="false" title="结算明细">
		<fx-details-table
			ref="detailsTable"
			v-fx-tab:loop.financial-manage_payment-order_details.settlementDetails
			showSummary
			:disabled="disabled"
...
const EDIT_INPUT_TYPE = ['settlementWay', 'accountName', 'payMoney', 'memo']
export default {
		...
		return {
			...
			tableColumn: [
				...
				{ prop: 'bankName', label: '开户银行' },
				{
					...
					label: '付款金额',
					editColumn: () => {
						return !this.disabled && !this.isPushDownSubPaymentOrder
					},
	...
	methods: {
		...
		onSettlementWayShow (row) {
			return () => {
				return new Promise(resolve => {
					...
					this.getSettlementWayList().then(res => {
		...
		accountNameRender (h, { row, $index }) {
			h = this.$createElement
			const filterKeys = ['accountName', 'bankAccount']
			return <fx-list-cell-select
		...
		onAccountNameShow (row) {
			return () => {
				return new Promise(resolve => {
					...
					this.getSettlementWayList().then(res => {
						this.accountNameList = (res.find(item => {
							return row.settleWay === item.settlementWay
						}) || { settlementValues: [] }).settlementValues.map(item => {
							return {
								...item,
								id: item.accountName,
		...
		payMoneyRender (h, { row, $index }) {
			...
			return <fx-list-cell-input-number
				ref={`payMoney-${$index}`}
				vModel={row.payMoney}
				isEdit={row.isEdit && !this.isPushDownSubPaymentOrder}
				row={row}
				rowIndex={$index}
				min={-Infinity}
				columnProp='payMoney'
		...
		memoRender (h, { row, $index }) {
			...
			return <fx-list-cell-input
				...
				on-on-input-enter={this.onMemoEnter}
			></fx-list-cell-input>
		},
		...
		onSaveDetailsHandler (params, origin) {
			if (params.id) {
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payment-order.updateDetails', this.createLoggerParams(params, origin))
				return this.$fxApi(`${apiName}.editDetail`, this.organDetails.id)({ data: params, logger })
			} else {
				const addItemInfo = this.getAddItemInfo(params)
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payment-order.addDetails', { addItemInfo, orderData: this.organDetails })
				return this.$fxApi(`${apiName}.addDetail`, this.organDetails.id, params)({ logger })
			}
		},
		...
		fpDeleteDetails (ids) {
			...
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.payment-order.deleteDetails', { deleteItemInfo, orderData: this.organDetails })
