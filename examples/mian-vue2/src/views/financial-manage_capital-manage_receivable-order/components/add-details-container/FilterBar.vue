	...
	<fx-list-filter @on-enter="onSearchClick">
		<template slot="row-1">
			<el-row>
				...
				<el-form-item label="客户支付方式" label-width="90px">
					<w-input
						:value="payWayName"
						disabled
						class="form-input"
			...
			<el-form-item prop="dateRang" label="出库时间">
				<w-date-picker
...
export default {
	...
	computed: {
		...
		sourceTypeName () {
			return this.$fxTypeMiddleware.getCommonTypeItemById('sourceTypeList')(this.organDetails.sourceType).name
		},
		payWayName () {
			return this.$fxTypeMiddleware.getCommonTypeItemById('payWayList')(this.organDetails.payWay).name
		}
	},
	methods: {
		onSearchClick () {
		...
		getFilterParams () {
			...
			this.filter.sourceType = this.organDetails.sourceType === 1000 ? ['empty'] : [this.organDetails.sourceType]
			this.filter.payWay && (this.filter.payWay = [this.organDetails.payWay])
