	...
	>
		...
		<div class="create-order">
			<w-scroll-bar :flex="isFullScreen">
				...
				<detailsInfo
					...
					:isRedReceivableOrder="isRedReceivableOrder"
					@on-details-refresh="getDetails"
				></detailsInfo>
			</w-scroll-bar>
		</div>
		...
		<template slot="footer-left">
			<fx-button
				...
				type="save"
				v-if="!isAdd"
				v-fx-tab:loop.financial-manage_receivable-order_details.refreshBtn
				@click="onRefresh"
			>刷新</fx-button>
			<fx-details-queue-tools
				ref="queueTools"
				v-fx-tab:loop.financial-manage_receivable-order_details.queueTools
				v-if="showBillQueue"
		...
		<template slot="footer-right">
			<fx-button
				ref="cancelBtn"
				v-fx-tab:loop.financial-manage_receivable-order_details.cancelBtn
				@click="onCancelClick"
			>取消</fx-button>
			<fx-button
				ref="saveBtn"
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.saveBtn
				@click="onSaveClick"
			>保存</fx-button>
			<fx-button
				ref="submitBtn"
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.submitBtn
			...
			<fx-button
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.returnBtn
			...
			<fx-button
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.checkBtn
			...
			<fx-button
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.uncheckBtn
			...
			<fx-button
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.recheckBtn
			...
			<fx-button
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.unrecheckBtn
			...
			<fx-button
				...
				v-fx-tab:loop.financial-manage_receivable-order_details.pushDownBtn
...
export default {
	...
	computed: {
		...
		showUncheckBtn () {
			return this.isState(['checked']) && this.currentState.type !== 'rechecked' && !this.isRedReceivableOrder
		},
		showRecheckBtn () {
			return this.isState(['checked']) && this.currentState.type !== 'rechecked' && !this.isRedReceivableOrder
		},
		...
		showPushDownBtn () {
			return this.isState(['checked', 'rechecked']) && !this.isRedReceivableOrder && false
		},
		moreBtnOption () {
			return [{
				show: !this.isAdd && this.isUnSubmit && !this.isRedReceivableOrder && this.detailsAuthGetter('store-manage_in-store-order.delete-instore'),
				label: '删除',
				icon: 'delete-trash',
				click: this.onDeleteClick
			}, {
				show: false && !this.isAdd && this.detailsAuthGetter('store-manage_in-store-order.export-instore'),
				label: '导出Excel',
				icon: 'export-excel',
				children: [{
				...
				show: false && !this.isAdd && this.detailsAuthGetter('store-manage_in-store-order.print-instore'),
				label: '打印',
				icon: 'print',
		...
		isRedReceivableOrder () {
			return this.isCommonTypeByRules('red-receivable-order')(this.organDetails.billType)
		}
	},
	created () {
		this.$fxEventBus.initEvent(this, 'app-visible', this.appVisibleHandler)
		this.$fxTabkey.init('financial-manage_receivable-order_details', this)
	},
	methods: {
		async add () {
			...
			this.organDetails = {
				...
				recheckState: 0,
				sysBillCode: '',
				userBillCode: this.$fxUtils.createBillCode('YS'),
				billType: 101,
				billBizTypeId: '',
		...
		onSaveOrganHandler () {
			if (this.isAdd) {
				...
				let logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.add', { orderData: this.organDetails, type: typeName })
				return this.$fxApi(`${apiName}.add`)({ data: this.organDetails }).then(res => {
					...
					this.fxDataVerification.resetData(this.organDetails)
					logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.add', { orderData: this.organDetails, type: typeName })
				...
				if (this.fxDataVerification.checkData(this.organDetails)) {
					...
					const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.update', this.createLoggerParams())
		...
		onDeleteHandler () {
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.delete', { orderData: this.organDetails })
		...
		onBuyerSubmitHandler () {
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.submit', { orderData: this.organDetails })
		...
		onBuyerReturnClick () {
			return this.$fxConfirm('是否确定退回？').then(() => {
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.return', { orderData: this.organDetails })
		...
		onBuyerCheckHandler () {
			const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.check', { orderData: this.organDetails })
		...
		onBuyerUncheckClick () {
			return this.$fxConfirm('是否确定反审？').then(() => {
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.uncheck', { orderData: this.organDetails })
		...
		onBuyerRecheckClick () {
			return this.$fxConfirm('是否确定复审？').then(() => {
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.recheck', { orderData: this.organDetails })
		...
		onBuyerUnrecheckClick () {
			return this.$fxConfirm('是否确定取消复审？').then(() => {
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.unrecheck', { orderData: this.organDetails })
		...
		onPushDownClick () {
			return this.$fxConfirm('是否确定下推？').then(() => {
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.pushdown', { orderData: this.organDetails })
		...
		exportData () {
			...
			// 		return false
			// 	} else {
			// 		const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.receivable-order.exportData', { orderData: this.organDetails })
