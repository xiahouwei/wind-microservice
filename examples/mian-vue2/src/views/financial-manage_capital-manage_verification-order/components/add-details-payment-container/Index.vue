...
export default {
	...
	methods: {
		...
		getSearchParams () {
			...
			const paymentIds = this.paymentTableDetails.map(item => {
				return item.paymentId
			})
			if (paymentIds.length > 0) {
				params.id = paymentIds
			}
		...
		getListDataHandler () {
			return this.$fxApi(`${apiName}.getPaymentOrderPage`, this.organDetails.id)({ data: this.getSearchParams() })
		},
		onCancelClick () {
		...
		onSaveClick () {
			this.$refs.listTable.getSelectedRows().then(selectRows => {
				...
				const logger = this.$fxLogger.createInfo('financial-manage.capital-manage.verification-order.addPaymentVerificationDetails', { orderData: this.organDetails, paymentOrders: selectRows })
				return this.$fxApi(`${apiName}.addPaymentVerificationDetails`, this.organDetails.id)({ data, logger }).then(() => {
					this.dialogVisible = false
