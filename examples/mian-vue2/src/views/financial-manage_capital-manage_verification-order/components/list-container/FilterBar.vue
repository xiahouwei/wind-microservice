...
export default {
	...
	data () {
		return {
			stateSearchList: this.$fxStateMiddleware.getStateConfig('verificationBillSearchState'),
			organIdList: [],
			otherOrganIdList: [],
			writeOffWayList: this.$fxStateMiddleware.getStateConfig('verificationWayFilter'),
			billTypeList: [],
			departmentList: [],
			writeOffManList: [],
			filter: {
	...
	methods: {
		...
		getFilterParams () {
			...
			// this.filter.operateMan = this.filter.writeOffMan
			return this.filter
		},
		getSelectData () {
			this.getOrganList()
