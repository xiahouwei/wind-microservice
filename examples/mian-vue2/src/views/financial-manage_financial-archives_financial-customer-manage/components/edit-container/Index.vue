<template>
	...
	>
		...
		<template slot="footer-right">
			<fx-button
				ref="cancelBtn"
				v-fx-tab:loop.financial-manage_financial-customer-manage_details.cancelBtn
				@click="onCancelClick"
			>取消</fx-button>
			<fx-button
				ref="saveBtn"
				v-fx-auth:basic-archives_organ-manage.edit-organ
				v-fx-tab:loop.financial-manage_financial-customer-manage_details.saveBtn
				type="primary"
				@click="onSaveClick"
			>保存</fx-button>
		</template>
...
export default {
	...
	created () {
		this.$fxTabkey.init('financial-manage_financial-customer-manage_details', this)
	},
	methods: {
		edit (customerInfo) {
			this.getOrganDetails(customerInfo.id).then(this.initSelect).then(() => {
				this.fxCache.clear()
		...
		onSaveOrganHandler () {
			...
			const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-customer-manage.update', this.createLoggerParams())
		...
		getOrganDetails (id) {
			return this.$fxApi(`${apiName}.getCustomerDetails`, id).then(res => {
