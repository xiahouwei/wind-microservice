<template>
  <w-collapse v-model="collapseVisible" contentHeight="320px" disabled :showArrow="false" title="结算信息">
    <fx-details-table
      ref="detailsTable"
      v-fx-tab:loop.financial-manage_financial-customer-manage_details.settlementInfo
      :disabled="disabled"
...
export default {
	...
	data () {
		return {
			...
			fxCache: this.$fxUtils.fxCache('financial-customer'),
	...
	methods: {
		...
		onSaveDetailsHandler (params, origin) {
			if (params.id) {
				const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-customer-manage.updateDetails', this.createLoggerParams(params, origin))
				return this.$fxApi(`${apiName}.editDetail`)({ data: params, logger })
			} else {
				const addItemInfo = this.getAddItemInfo(params)
				const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-customer-manage.addDetails', { name: this.organDetails.name, addItemInfo })
		...
		fpDeleteDetails (ids) {
			...
			const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-customer-manage.deleteDetails', { deleteItemInfo, name: this.organDetails.name })
