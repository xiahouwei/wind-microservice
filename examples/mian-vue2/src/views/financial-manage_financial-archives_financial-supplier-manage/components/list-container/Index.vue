...
export default {
	...
	data () {
		return {
			tableColumn: [
				{ prop: 'code', label: '供货商编号', required: true },
				{ prop: 'name', label: '供货商名称', required: true },
				{ prop: 'cwSupplierType.name', label: '供货商类别' },
				{
					prop: 'enableFlag',
					label: '状态',
					required: true,
	...
	methods: {
		...
		getSearchParams () {
			...
			return {
				...
				...this.$fxCommon.getSearchParams('financialSupplierList')(params)
			}
		},
		getListData () {
			return this.$fxApi(`${apiName}.getSupplierList`)({ data: this.getSearchParams() }).then(res => {
		...
		onEnableClick () {
			this.$refs.organList.getSelectedRows().then(res => {
				this.$fxConfirm('是否确定启用所选供货商？').then(() => {
		...
		onEnableHandler (selectRows) {
			...
			const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-supplier-manage.enable', { names: this.getSupplierName(selectRows) })
		...
		onDisableClick () {
			this.$refs.organList.getSelectedRows().then(res => {
				this.$fxConfirm('是否确定停用所选供货商？').then(() => {
		...
		onDisableHandler (selectRows) {
			const params = this.createMulityParams(selectRows)
			const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-supplier-manage.disable', { names: this.getSupplierName(selectRows) })
		...
		onExportDataClick () {
			// const logger = this.$fxLogger.createInfo('financial-manage.financial-archives.financial-supplier-manage.exportData')
