	...
	<w-app-container class="print-view" direction="row">
		...
		<div ref="printContainer" :style="containerStyles" class="print-view__container">
			<template v-for="(pageItem, pageIndex) in listData">
				...
				>
					<div ref="printTile" class="print-view__title" :class="{'print-view__title__is-low': isStylusPrinter}">{{billName}}</div>
		...
		<div class="print-view__form-tools">
			...
			<div class="print-view_column-filter print-view__box">
				<span class="columnName">{{subColumnName}}</span>
				<div v-for="(item, index) in subColumns.filter(item => item.prop)" :key="index" class="print-view_column-filter-item">
					<el-checkbox
						v-model="item.visible"
						:label="item.label"
						:disabled="item.disabled"
...
export default {
	...
	computed: {
		...
		systemParamPrintSumIsSumBill () {
			return this.getSysParams('XTGF_TWOTHREE_PRINT_SUM', 'value')
		},
		containerStyles () {
	...
	methods: {
		getRouteParams () {
			...
			this.$fxAppTabs.setTabTitle({ billName: this.billName })
			...
			return this.$fxApi(`${this.apiName}.${this.billType}`)({ data: this.billId }).then(res => {
				...
				this.listData.forEach(item => {
					item.details.map(detail => {
						detail.groupType = this.tableColumn[0].groupType
					})
				})
				return Promise.resolve(res)
			})
		...
		initTableColumn (res) {
			const filterColumMap = res.globalConstantMap
			const columnData = res.all.details.filter(item => {
				return item.groupType === 'DETAIL_PRODUCT'
			})
			...
			columnData.forEach(item => {
				...
				item.additionFlag = false
			})
			this.columnName = this.normalizationValueByValueMap(columnData[0].groupType, filterColumMap)
			if (!this.isInited) {
				this.createSubColumns(res, visiableColumns, columnData)
				this.isInited = true
			} else {
				this.setSubColumnsVisible(this.subColumns, visiableColumns)
			}
			this.tableColumn = columnData
			this.subColumnName = this.normalizationValueByValueMap(this.subColumns[0].groupType, filterColumMap)
			this.tableColumnChange()
		},
		createSubColumns (res, visiableColumns, columnData) {
			const subColumns = res.all.details.filter(item => {
				return item.groupType === 'DETAIL_DOSING'
			})
			this.setSubColumnsVisible(subColumns, visiableColumns)
			for (let i = 0; i < columnData.length; i++) {
				subColumns.push({ prop: '', label: '', groupType: 'DETAIL_DOSING' })
			}
			this.subColumns = subColumns
		},
		setSubColumnsVisible (subColumns, visiableColumns) {
			subColumns.filter(item => item.field).forEach(item => {
				item.disabled = false
				const index = item.field.indexOf('.')
				item.prop = item.field.slice(index + 1, item.field.length)
				const col = visiableColumns.find(visiableItem => {
					return visiableItem.field === item.field
				})
				if (col) {
		...
		tableColumFormatter (columnItem) {
			return (row) => {
				const index = columnItem.field.indexOf('.')
				const cellValue = columnItem.groupType === 'DETAIL_PRODUCT' ? this.getCellText(row, columnItem.field) : this.getCellText(row, columnItem.field.slice(index + 1, columnItem.field.length))
				if (columnItem.dataType === 'List') {
					const listValue = this.getCellText(row, 'otherChargeType')
					return this.normalizationValueByList(listValue, columnItem.dataValue)
				} else if (cellValue === '') {
					return '---'
				} else {
		...
		printState () {
			const typeObj = {
				InStore: 'in_store',
				OutStore: 'out_store',
				MoveApply: 'move_apply',
				CheckBill: 'check',
				StockPlan: 'stock_plans',
				StockOrder: 'stock_order',
				SalesOrder: 'sales_order',
				MachineCompose: 'machine_compose'
			}
			return this.$fxApi(`${this.apiName}.printState`, typeObj[this.billType])({ data: this.billId }).then(res => {
				return Promise.resolve(res)
			})
		},
		printHandler () {
		...
		normalizationValueByList (value, dataValue) {
			return value.length > 0 ? value.map(item => item[dataValue]).join(',') : '---'
		},
		...
		createForm (listData) {
			...
			if (this.isStylusPrinter) {
				formEl.setAttribute('style', `width:${this.currentPrintSizeObj.width}px;margin: 0 auto 5px`)
			}
			return formEl
		},
		createFooter (listData, pageNum, pageTotal) {
			...
			if (this.isStylusPrinter) {
				...
				footerEl.setAttribute('style', `width:${this.currentPrintSizeObj.width}px;margin: 5px auto 20px`)
			}
			return footerEl
		},
		createTable (listData, listPageIndex, startIndex, endIndex) {
			...
			// subheadtr.classList.add('print-view__table-tr')
			// subheadtr.setAttribute('style', 'background-color: #eee')
			// const _subColumns = this.$fxUtils.deepClone(this.subColumns.filter(item => item.visible && item.prop))
			// const diffLen = _tableColumn.length - _subColumns.length - 1
			// for (let i = 0; i < diffLen; i++) {
			// 	_subColumns.push({ prop: '', label: '', groupType: 'DETAIL_DOSING' })
			// }
			// _subColumns.unshift({
			// 	prop: 'index',
			// 	label: '配料序号'
			// })
			// _subColumns.forEach(item => {
			// 	const td = document.createElement('th')
			// 	const div = document.createElement('div')
			// 	div.classList.add('print-view__table-cell')
			// 	if (item.prop === 'index') {
			...
			// 	}
			// 	td.appendChild(div)
			// 	subheadtr.appendChild(td)
			// })
			thead.appendChild(headtr)
			// thead.appendChild(subheadtr)
			...
			details.forEach((listItem, listIndex) => {
				...
				if (listItem.dosingDetails.length > 0) {
					const subheadtr = document.createElement('tr')
					subheadtr.classList.add('print-view__table-tr')
					subheadtr.setAttribute('style', 'background-color: #eee')
					const _subColumns = this.$fxUtils.deepClone(this.subColumns.filter(item => item.visible && item.prop))
					...
					tbody.appendChild(subheadtr)
					let subDetails = listItem.dosingDetails
					if (this.isStylusPrinter) {
						subDetails = listItem.dosingDetails.slice(startIndex, endIndex)
					}
					subDetails.forEach((listItem, listIndex) => {
						const bodysubtr = document.createElement('tr')
						bodysubtr.classList.add('print-view__table-tr')
						bodysubtr.setAttribute('style', 'background-color: #f6f6f6')
						_subColumns.forEach(item => {
							const td = document.createElement('td')
							const div = document.createElement('div')
							div.classList.add('print-view__table-cell')
							...
							td.appendChild(div)
							bodysubtr.appendChild(td)
							tbody.appendChild(bodysubtr)
						})
					})
				}
			})
		...
		renderTableCellValueMap (div, listItem, listIndex, cellItem) {
			const index = cellItem.field.indexOf('.')
			const cellValue = listItem.groupType === 'DETAIL_PRODUCT' ? listItem[cellItem.field] : listItem[cellItem.field.slice(index + 1, cellItem.field.length)]
			div.innerHTML = this.normalizationValueByNumberType(this.normalizationValueByValueMap(cellValue, cellItem.valueMap), cellItem.numberType)
		},
		renderTableCellList (div, listItem, listIndex, cellItem) {
			const listValue = this.getCellText(listItem, 'otherChargeType')
			div.innerHTML = this.normalizationValueByList(listValue, cellItem.dataValue)
		},
		renderTableCell (div, listItem, listIndex, cellItem) {
			div.innerHTML = this.normalizationValueByNumberType(this.getCellText(listItem, cellItem.prop), cellItem.numberType) ? this.normalizationValueByNumberType(this.getCellText(listItem, cellItem.prop), cellItem.numberType) : '---'
		},
		...
		tableColumnChange () {
			if (this.subColumns.filter(item => item.visible && item.prop).length >= this.tableColumn.filter(item => item.visible).length) {
				this.subColumns.filter(item => !item.visible && item.prop).forEach(item => {
					item.disabled = true
				})
				this.tableColumn.filter(item => item.visible).forEach(item => {
					item.disabled = true
				})
			} else {
				this.subColumns.filter(item => !item.visible && item.prop).forEach(item => {
			...
			this.tableColumn.find(item => item.prop === 'item.name').disabled = true
			this.subColumns.find(item => item.prop === 'item.name').disabled = true
		},
		onPrePageClick () {
		...
		getTemplateParams (name) {
			...
			const cols = this.$refs.detailsTable[0].getVisibleColumns()
			let subCols = [...this.subColumns]
			cols.forEach(item => {
				item.field = item.prop
			})
			subCols = subCols.filter((item) => {
				return item.visible && !!item.prop
			})
			return {
				name,
				billType: this.billType,
				formData: {
					details: [...cols, ...subCols],
