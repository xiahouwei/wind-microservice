...
export default {
	...
	data () {
		return {
			...
			machiningList: this.$fxTypeMiddleware.getCommonTypeList('machiningType'),
			filter: {
				dateRang: [],
				userBillCode: '',
				billState: [],
	...
	methods: {
		...
		getOrganList () {
			if (!this.organIdList.length) {
				...
				const typeList = ['1']
				return this.$fxApi('machiningOrder.getOrganSelectTab', typeList)({ loading: false }).then(res => {
		...
		organIdChange () {
			this.filter.productHouse = []
			this.filter.dosingHouse = []
			this.getHouseList()
		},
		onHouseSelectShow () {
		...
		getHouseList () {
			...
			const typeList = [1]
			return this.$fxApi('machiningOrder.getHouseSelect', typeList)({ data: params, loading: false }).then(res => {
				this.houseList = res
				return Promise.resolve()
			})
		},
		...
		getItemSelectList () {
			...
			return this.$fxApi('machiningOrder.getItemSelectTab').then(res => {
				this.itemList = res
		...
		// 经办人
		getAgentList () {
			return this.$fxApi('machiningOrder.getAgentList')({ data: [] }).then(res => {
