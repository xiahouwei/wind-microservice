<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1300px" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="1" type="MachiningOrder"></fx-mistake-check-step>
			<div class="mistake-check-warning fx-warn-font">注：以下明细中，存在成品实际产量为0或配料实际用量为0的品项，请修改数量或删除！</div>
...
const EDIT_PRODUCT_TYPE = ['actualTotal']
const EDIT_ADDITION_TYPE = ['actualTotal']
	...
	methods: {
			...
			return new Promise(resolve => {
				this.$refs.detailsTable.checkDetailsUnSaving().then(() => {
					...
					if (this.tableData.length === 0) {
						...
						return `${item.actualTotal}` === '0' && !item.additionFlag
					})) {
						this.$fxMessage.warning('成品实际产量不能为0')
					} else if (this.tableData.some(item => {
						return `${item.actualTotal}` === '0' && item.additionFlag
					})) {
						this.$fxMessage.warning('附成品实际产量不能为0')
					} else if (dosingDetails.some(item => {
						return `${item.actualTotal}` === '0'
					})) {
						this.$fxMessage.warning('配料实际用量不能为0')
					} else {
						resolve()
					}
				})
		...
		getCountBomMachineCharge (row) {
			this.$fxApi('machiningOrder.getCountBomMachineChargeConversion', row.composeBom.id, row.unit.id).then(res => {
				...
				row.makeCharge = res.machineChargeConversion * row.actualTotal
			})
		},
		...
		otherChargeTypeRender (h, { row, $index }) {
			...
			if (!row.additionFlag) {
				return <fx-list-cell-select-mulity
					...
					cellText={row.chargeTypeName}
					rowIndex={$index}
					columnProp='otherChargeTypeList'
					dropdownWidth='180px'
					selectOptionList={this.otherChargeTypeList}
					on-on-change={this.onOtherChargeTypeChange}
				></fx-list-cell-select-mulity>
			} else {
				return <div ref={`otherChargeTypeList-${$index}`}>{}</div>
			}
		},
		onOtherChargeTypeChange (item, row) {
			const chargeTypeName = []
			item.forEach((type) => {
				for (const ChargeType of this.otherChargeTypeList) {
					if (ChargeType.id === type.id) {
						chargeTypeName.push(ChargeType.name)
						return
					}
				}
			})
			row.chargeTypeName = chargeTypeName.join(',')
		},
