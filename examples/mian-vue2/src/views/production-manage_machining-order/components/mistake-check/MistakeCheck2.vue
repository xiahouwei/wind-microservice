<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1300px" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="2" type="MachiningOrder"></fx-mistake-check-step>
			<div class="mistake-check-warning fx-warn-font">注：以下明细中，存在配料仓库为空，请选择配料仓库！</div>
	...
	methods: {
		...
		manualChargeRender (h, { row, $index }) {
			...
			if (!row.additionFlag) {
				...
				return <div ref={`manualCharge-${$index}`}>{this.$fxCommon.toFixedAutoZeroFill(row.makeCharge, this.sysMoneyPointSize)}</div>
			}
		},
		onManualChargeEnter () {
