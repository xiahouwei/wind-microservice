<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1300px" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="3" type="MachiningOrder"></fx-mistake-check-step>
...
const EDIT_ISHAND_TYPE = ['house', 'code']
const EDIT_PRODUCT_TYPE = ['house', 'code', 'bomCode']
const EDIT_PLAN_TYPE = ['house', 'actualTotal', 'makeCharge', 'manualCharge', 'otherChargeTypeList', 'otherCharge', 'taxRate', 'memo']
const EDIT_ADDITION_TYPE = ['house', 'code', 'unit', 'actualTotal', 'productionCostPrice', 'productionCost', 'taxRate', 'memo']
const EDIT_SUB_TYPE = ['subHouse', 'subCode', 'subUnit']
export default {
	...
	methods: {
		...
		onDetailsRefresh () {
			return this.$fxApi('machiningOrder.Belong', this.organDetails.id).then(res => {
		...
		codeRender (h, { row, $index }) {
			...
			return <fx-list-cell-select
				...
				on-on-input-tab={this.onCodeSelectTab}
			></fx-list-cell-select>
		},
		...
		setComposeBom (row, item) {
			return this.getBomList(item.id).then(res => {
				...
				if (this.bomList.length > 0) {
					...
					if (row.composeBom) {
						this.getLastDosing(this.organDetails.organ.id, row.composeBom.id, row.planTotal, row.composeBom.unitId).then(res => {
		...
		bomSelectChange (item, row, rowIndex) {
			this.onBomChangeConfirm(row.id).then(() => {
				...
				if (!row.additionFlag) {
					...
					this.getLastDosing(this.organDetails.organ.id, item.id, row.planTotal, item.unitId).then(res => {
						row.dosingDetails = res
						row.dosingDetails.map(item => {
							item.actualTotal = item.bomProportion * row.actualTotal
							item.theoryTotal = item.bomProportion * row.actualTotal
