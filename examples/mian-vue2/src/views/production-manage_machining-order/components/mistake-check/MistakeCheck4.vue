<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1300px" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="4" type="MachiningOrder"></fx-mistake-check-step>
			<div class="mistake-check-warning fx-warn-font">注：以下成品下的附成品、配料明细中，存在重复品项，或两个相同成品下的BOM、配料完全一样，为成品品项重复，请删除重复品项！</div>
...
export default {
	...
	data () {
		return {
			...
			dialogVisible: false,
			tableColumn: [
				{ prop: 'house.code', label: '仓库编号', width: '120px' },
	...
	methods: {
		...
		// 品项重复
		onDetailsRefresh () {
			return this.$fxApi('machiningOrder.repeatItem', this.organDetails.id).then(res => {
