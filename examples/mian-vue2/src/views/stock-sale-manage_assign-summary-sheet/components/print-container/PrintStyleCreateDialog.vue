<template>
	<w-dialog
		...
		width="400px"
		:title="`${dialogTitle}打印样式`"
	>
		<div class="create-area">
			<el-form ref="form" :model="printStyle" :rules="rules" size="mini" inline label-width="110px">
				<el-form-item prop="name" label="样式名称：">
					<w-input
						ref="name"
						v-model="printStyle.name"
...
export default {
	...
	methods: {
		open (printStyleName, dialogType) {
			this.printStyle.name = printStyleName
		...
		onSaveHandler () {
			if (this.dialogType === 'del') {
				this.$emit('on-del-style', this.printStyle.name)
				this.dialogVisible = false
			} else {
				this.$emit('on-save-style', this.printStyle.name)
				this.dialogVisible = false
			}
		},
		onCancelClick () {
