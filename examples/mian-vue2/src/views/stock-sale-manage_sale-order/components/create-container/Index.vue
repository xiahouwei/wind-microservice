<template>
	...
	>
		<template slot="sub">
			...
			<fx-details-state-tag :label="inAssignStatus.label" :icon="inAssignStatus.icon"></fx-details-state-tag>
			<fx-details-state-tag :label="outAssignStatus.label" :icon="outAssignStatus.icon"></fx-details-state-tag>
		</template>
		...
		<div class="create-organ">
			<w-scroll-bar :flex="isFullScreen">
				<basicInfo
					...
					:disabled="isDisable || hasParentBill || authDisabled || isJsdOrder"
				...
				<detailsInfo
					...
					:disabled="isDisable || authDisabled || isJsdOrder"
					:deleteDisabled="isDisable || isJsdOrder"
					:itemName="itemName"
					@on-details-refresh="onDetailsRefresh"
		...
		<template slot="footer-left">
			<fx-button
				...
				type="save"
				v-if="!isAdd"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.refreshBtn
				@click="onRefresh"
			>刷新</fx-button>
		</template>
		<template slot="footer-right">
			<el-button
				ref="cancelBtn"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.cancelBtn
				@click="onCancelClick"
			>取消</el-button>
			<fx-button
				ref="saveBtn"
				type="save"
				v-fx-auth:stock-sale-manage_sale-order.edit-sale="!isDisable"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.saveBtn
				@click="onSaveClick"
			>保存</fx-button>
			<fx-button
				ref="submitBtn"
				type="submit"
				v-fx-auth:stock-sale-manage_sale-order.submit-sale="isState(['unSubmit'])"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.submitBtn
				@click="onBuyerSubmitClick"
			>提交</fx-button>
			<fx-button
				ref="returnBtn"
				type="return"
				v-fx-auth:stock-sale-manage_sale-order.back-sale="isState(['unChecked'])"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.returnBtn
				@click="onBuyerReturnClick"
			>退回</fx-button>
			<fx-button
				ref="checkBtn"
				type="check"
				v-fx-auth:stock-sale-manage_sale-order.check-sale="isState(['unSubmit', 'unChecked'])"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.checkBtn
				@click="onBuyerCheckClick"
			>审核</fx-button>
			<fx-button
				ref="uncheckBtn"
				type="uncheck"
				v-fx-auth:stock-sale-manage_sale-order.uncheck-sale="isState(['checked']) && !isRechecked && !isCarryForward && !isSrmOrder"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.uncheckBtn
				@click="onBuyerUncheckClick"
			>反审</fx-button>
			<fx-button
				ref="recheckBtn"
				type="recheck"
				v-fx-auth:stock-sale-manage_sale-order.recheck-sale="isState(['checked']) && !isRechecked && !isCarryForward"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.recheckBtn
				@click="onBuyerRecheckClick"
			>复审</fx-button>
			<fx-button
				ref="unrecheckBtn"
				type="unrecheck"
				v-fx-auth:stock-sale-manage_sale-order.unrecheck-sale="isRechecked && !isCarryForward"
				v-fx-tab:loop.stock-sale-manage_sale-order_details.unrecheckBtn
...
export default {
	...
	computed: {
		...
		moreBtnOption () {
			return [{
				show: !this.isAdd && !this.isChecked && !this.hasParentBill && this.detailsAuthGetter('stock-sale-manage_sale-order.delete-sale'),
				label: '删除',
				icon: 'delete-trash',
				click: this.onDeleteClick
			}, {
				show: this.detailsAuthGetter('stock-sale-manage_sale-order.export-blank|export-sale'),
				label: '导出Excel',
				icon: 'export-excel',
				children: [{
					show: this.detailsAuthGetter('stock-sale-manage_sale-order.export-blank'),
					label: '导出空白模板',
					icon: 'export-template',
					click: this.exportModel
				}, {
					show: this.detailsAuthGetter('stock-sale-manage_sale-order.export-sale'),
					label: '导出明细数据',
					icon: 'export-data',
					click: this.exportData
				}]
			}, {
				show: !this.isDisable && !this.hasParentBill && this.detailsAuthGetter('stock-sale-manage_sale-order.import-sale'),
				label: '导入Excel',
				icon: 'import-excel',
				children: [{
					label: '按模板导入数据',
				...
				show: !this.isAdd && this.detailsAuthGetter('stock-sale-manage_sale-order.copy-sale') && !this.isJsdOrder,
				label: '复制单据',
				icon: 'copy-order',
				click: this.onCopyClick
			}, {
				show: !this.isAdd && this.detailsAuthGetter('stock-sale-manage_sale-order.print-sale'),
				label: '打印',
				icon: 'print',
				click: this.onPrintClick
			}, {
				show: !this.isAdd && this.isState(['checked', 'rechecked']) && this.detailsAuthGetter('stock-sale-manage_sale-order.assign-in-sale') && !this.isSrmOrder,
				disabled: this.inAssignBtnDisabled,
				label: '入库分派',
				icon: 'instore-assign',
				click: this.onInAssignClick
			}, {
				show: !this.isAdd && this.isState(['checked', 'rechecked']) && this.detailsAuthGetter('stock-sale-manage_sale-order.assign-out-sale') && !this.isSrmOrder,
				disabled: this.outAssignBtnDisabled,
				label: '出库分派',
				icon: 'outstore-assign',
				click: this.onOutAssignClick
			}]
		},
		isAdd () {
		...
		inAssignStatus () {
			return this.$fxStateMiddleware.getStateObj('inAssignState')(this.organDetails.inAssignStatus)
		},
		outAssignStatus () {
			return this.$fxStateMiddleware.getStateObj('outAssignState')(this.organDetails.outAssignStatus)
		},
		isState () {
			return (type) => {
		...
		inAssignBtnDisabled () {
			return this.$fxStateMiddleware.isState('inAssignState')(['assigned'])(this.organDetails.inAssignStatus) || this.isCommonTypeByRules('assign-sub')(this.organDetails.billType, this.organDetails.subBillType) || this.isNoAssign
		},
		outAssignBtnDisabled () {
			return this.$fxStateMiddleware.isState('outAssignState')(['assigned'])(this.organDetails.outAssignStatus)
		},
		authDisabled () {
		...
		isSrmOrder () {
			return this.isCommonTypeByRules('srm-sub')(this.organDetails.generateWay)
		},
		isNoAssign () {
			return this.inAssignStatus.id === 3
		}
	},
	created () {
		this.$fxEventBus.initEvent(this, 'app-visible', this.appVisibleHandler)
		this.$fxTabkey.init('stock-sale-manage_sale-order_details', this)
		this.initBill()
	...
	methods: {
		...
		async add () {
			...
			const userOrgan = this.$fxUtils.getSelectTagById(currentOrgan.id, this.sellerSelectList)
			this.organDetails = {
				billState: 0,
				sysBillCode: '',
				userBillCode: this.$fxUtils.createBillCode('XS'),
				billType: 5000,
				billBizType: {
					id: ''
				},
		...
		getBusinessTypeList (organDetails = { billType: 5000 }) {
		...
		getSellerList (organDetails) {
			const data = {
				billType: 'SalesOrder',
				otherOrganId: organDetails ? organDetails.otherOrgan.id : '',
		...
		getPlanList (organDetails) {
			const params = {
				...
				buyerOrganId: organDetails.otherOrgan.id,
				sellerOrganId: organDetails.organ.id,
				urgent: organDetails.urgent,
				planIdList: [organDetails.stockPlan.id || '']
			}
		...
		deleteEmptyBill () {
			if (this.organDetails.id && (this.tableDetails.length === 0 || (this.tableDetails.length === 1 && !this.tableDetails[0].id)) && !this.hasParentBill) {
		...
		onSaveOrganHandler () {
			if (this.isAdd) {
				...
				let logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.add', { orderData: this.organDetails, type: typeName })
				return this.$fxApi(`${apiName}.add`)({ data: this.organDetails }).then(res => {
					...
					this.fxDataVerification.resetData(this.organDetails)
					logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.add', { orderData: this.organDetails, type: typeName })
				...
				if (this.fxDataVerification.checkData(this.organDetails)) {
					...
					const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.update', this.createLoggerParams())
		...
		onDeleteHandler () {
			...
			const logger = this.$fxLogger.createInfo(`stock-sale-manage.sale-order.${del}`, { orderData: this.organDetails })
		...
		onBuyerSubmitHandler () {
			const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.submit', { orderData: this.organDetails })
		...
		onBuyerReturnClick () {
			return this.$fxConfirm('是否确定退回？').then(() => {
				const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.return', { orderData: this.organDetails })
		...
		onBuyerCheckHandler () {
			const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.check', { orderData: this.organDetails })
			return this.$fxApi(`${apiName}.check`, this.organDetails.id)({ logger }).then(async res => {
				await this.getPlanList(res)
				this.updateBillState(res)
				this.fxDataVerification.resetData(this.organDetails)
				this.tableDetails = res.details
				return Promise.resolve()
		...
		onBuyerUncheckClick () {
			return this.$fxConfirm('是否确定反审？').then(() => {
				const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.uncheck', { orderData: this.organDetails })
		...
		onBuyerRecheckClick () {
			return this.$fxConfirm('是否确定复审？').then(() => {
				const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.recheck', { orderData: this.organDetails })
		...
		onBuyerUnrecheckClick () {
			return this.$fxConfirm('是否确定取消复审？').then(() => {
				const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.unrecheck', { orderData: this.organDetails })
		...
		updateBillState ({ billState, inAssignStatus, outAssignStatus, recheckState, stockPlan = {}, submitMan = {}, submitDate = '', checkMan = {}, checkDate = '', systemMemo = '' }) {
			this.organDetails.billState = billState
			this.organDetails.inAssignStatus = inAssignStatus
			this.organDetails.outAssignStatus = outAssignStatus
			...
			this.$set(this.organDetails, 'stockPlan', stockPlan)
		},
		onOrderSave () {
		...
		exportModel () {
			this.onSaveClick().then(() => {
				const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.exportModel', { orderData: this.organDetails })
		...
		exportData () {
			this.onSaveClick().then(() => {
				...
				const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.exportData', { orderData: this.organDetails })
		...
		importExcelHandler (file) {
			...
			const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.importModel', { orderData: this.organDetails, name: file.name })
		...
		onPrintClick () {
			this.$fxGoPrint('SalesOrder', this.organDetails.id)
		},
		onCopyClick () {
			const logger = this.$fxLogger.createInfo('stock-sale-manage.sale-order.copy', { orderData: this.organDetails })
		...
		onInAssignClick () {
			this.onAssignClick('in')
		},
		onOutAssignClick () {
			this.onAssignClick('out')
		},
		onAssignClick (type) {
			this.assignCheck(type).then(() => {
				const path = type + 'StoreAssign'
				this.$fxAppTabs.tabsPush({
					path,
					query: {
		...
		assignCheck (type) {
			return this.$fxApi(`${apiName}.assignCheck`, this.organDetails.id, type)
		},
		onBillChainClick () {
			return this.$fxApi(`${apiName}.getBillChain`, this.organDetails.id).then(res => {
				...
				const billData = {
					...
					inAssignStatus: this.organDetails.inAssignStatus,
					outAssignStatus: this.organDetails.outAssignStatus,
					id: this.organDetails.id,
