<template>
	<w-dialog v-model="dialogVisible" width="1200px" title="异常警告" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="2" type="SalesOrder"></fx-mistake-check-step>
	...
	methods: {
		...
		getRepetitive (tableDetails) {
			...
			tableDetails.forEach(item => {
				const key = `${item.item.id}.${item.giftFlag}`
				if (!repetMap[key]) {
					repetMap[key] = [item]
				} else {
		...
		getDeleteItemInfo (ids) {
			return this.tableData.filter(item => {
				...
				return `品项名称:${name},单位:${itemUnit},销售数量:${amount},赠品标记:${giftFlag}`
			}).join(';')
		},
		clear () {
