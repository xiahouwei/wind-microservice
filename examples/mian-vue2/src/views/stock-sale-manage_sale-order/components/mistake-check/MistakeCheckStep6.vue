<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1200px" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="4" type="SalesOrder"></fx-mistake-check-step>
...
export default {
	...
	computed: {
		...
		computedTableColumn () {
			...
			const costPower = ['price', 'money', 'taxPrice', 'taxMoney', 'taxRate', 'taxRateMoney']
	...
	methods: {
		...
		getSupplierRefItem () {
			return this.$fxApi(`${apiName}.getSupplierRefItem`, this.organDetails.id).then(res => {
		...
		onDeleteClick (delData) {
			...
			return this.$fxApi(`${apiName}.delCheckDetailsMulity`, this.organDetails.id, true)({ data: delData, logger }).then(res => {
