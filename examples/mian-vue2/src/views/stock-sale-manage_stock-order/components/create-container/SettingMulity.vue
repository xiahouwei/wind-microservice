...
export default {
	...
	computed: {
		...
		computedColumnSelectList () {
			let columnSelectList = this.columnSelectList
			if (this.organDetails.organ.taxPlayerFlag !== 1 || !this.haveMoneyPower) {
				columnSelectList = columnSelectList.filter(item => {
					return item.id !== 0
				})
			}
			if (this.isAssign) {
	...
	methods: {
		onSaveClick () {
			if (this.settingKey === 1 && this.$fxUtils.compareDate(this.organDetails.orderDate, this.expectArrivalDate)) {
		...
		refreshFormData () {
			this.settingKey = (this.organDetails.organ.taxPlayerFlag === 1 && !!this.haveMoneyPower) ? 0 : this.isAssign ? 2 : 1
			this.tax = 0
