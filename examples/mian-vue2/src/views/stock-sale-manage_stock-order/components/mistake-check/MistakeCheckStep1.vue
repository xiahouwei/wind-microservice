<template>
	<w-dialog v-model="dialogVisible" width="1400px" title="异常警告" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="1" type="StockOrder"></fx-mistake-check-step>
...
const EDIT_INPUT_TYPE = ['storeCode', 'amount', 'taxPrice', 'taxMoney', 'taxRate', 'price', 'money', 'assistUnitAmount', 'expectArrivalDate']
...
export default {
	...
	computed: {
		...
		computedEditColumn () {
			const moneyPower = ['taxPrice', 'taxMoney', 'taxRate', 'price', 'money']
			let editColumn = EDIT_INPUT_TYPE
			if (this.isSingleHouse) {
				editColumn = editColumn.filter(item => item !== 'storeCode')
			}
			if (!this.haveMoneyPower) {
				editColumn = editColumn.filter(item => !moneyPower.includes(item))
			}
			return editColumn
		},
		isSingleHouse () {
	...
	methods: {
		open (tableData) {
			this.tableData = tableData.map(item => {
				...
				if (item.stockPlanDetail) {
					arrivalDate = new Date(this.organDetails.orderDate).getTime() + this.$fxUtils.h2ms(item.stockPlanDetail.arrivalCycle)
				} else {
					arrivalDate = new Date(this.organDetails.orderDate).getTime() + this.$fxUtils.h2ms(this.sysArrivalInterval)
				}
				const { taxPrice, taxRate, price, itemUnit } = item
				const mainUnitId = itemUnit.id
				const inHouse = this.isSingleHouse ? { id: this.organDetails.inHouse } : {
					code: '请选择',
					name: '请选择'
				}
				return {
		...
		getAddItemInfo () {
			return this.tableData.map(item => {
				let storeName = ''
				if (this.isSingleHouse) {
					storeName = item.storeSelectList.find(_item => _item.id === this.organDetails.inHouse).name
				} else {
