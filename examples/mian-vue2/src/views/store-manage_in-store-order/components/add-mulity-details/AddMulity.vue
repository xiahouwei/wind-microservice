...
export default {
	...
	methods: {
		async open () {
			...
			let listData = []
			if (this.store) {
				if (this.isHandCreate && this.isSupplier) {
					listData = await this.getSupplierItemListMulity(this.store, this.organDetails.otherOrgan.id)
				} else {
					listData = await this.getItemListMulity(this.store)
				}
			}
		...
		storeChangeHandler (id) {
			this.listSelectIds = []
			if (this.isHandCreate && this.isSupplier) {
				this.getSupplierItemListMulity(id, this.organDetails.otherOrgan.id).then(res => {
					this.listData = res
					this.store = id
		...
		getSupplierItemListMulity (storeId, supplierId) {
			return this.$fxApi('inStore.getSupplierItemListMulity', this.organDetails.id, storeId, supplierId)
		},
		...
		onSaveClick (close = true) {
			...
			const logger = this.$fxLogger.createInfo('store-manage.in-store-order.addMulityDetails', { orderData: this.organDetails, addItemListInfo })
