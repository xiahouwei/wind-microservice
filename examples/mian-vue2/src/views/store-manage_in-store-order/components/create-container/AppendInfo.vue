<template>
	<w-collapse ref="collapse" v-model="collapseVisible" title="附加信息">
		<el-form size="mini" label-width="100px" inline label-position="left">
			...
			<fx-details-form-row>
				<el-form-item label="附件资料：" :required="appendixesRequired">
					<fx-upload :fileList="fileList" :limit="9" :disabled="isBillStateDisable" class="upload-content" @on-change="onFileChange"></fx-upload>
				</el-form-item>
			</fx-details-form-row>
			<fx-details-form-row>
...
export default {
	...
	computed: {
		...mapGetters(['getOrderDataIsRequired']),
		fileList () {
			return (this.organDetails.appendixes || []).map(item => {
		...
		appendixesRequired () {
			if (this.isReturn) {
				return this.getOrderDataIsRequired('InStore', this.organDetails.billType, 'appendixes', this.organDetails.inOrgan.organType)
			}
			return this.isHand && this.getOrderDataIsRequired('InStore', this.organDetails.billType, 'appendixes')
		},
		isBillStateDisable () {
	...
	methods: {
		onFileChange (list) {
			this.organDetails.appendixes = list.map(item => {
				return {
					appendixUrl: item
				}
			})
