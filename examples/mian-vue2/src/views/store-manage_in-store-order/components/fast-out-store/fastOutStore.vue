<template>
	<w-dialog v-model="dialogVisible" width="1200px" title="选择快速出库品项" heightFullscreen @close="onClose">
...
const EDIT_INPUT_TYPE = ['quickAmount']
			...
			return this.checkDetailsUnSaving().then(() => {
				return this.$refs.detailsTable.getSelectRows(false).then(res => {
					...
					if (selectRows.length <= 0) {
						return this.$fxMessage.warning('请选择快速出库品项')
					}
					const isZero = selectRows.some(item => {
						return item.quickAmount === 0
					})
					if (isZero) {
						return this.$fxMessage.warning('所选品项快速出库数量不能为0')
					}
					const data = this.$fxUtils.deepClone(this.organDetails)
					data.details = res
					const logger = this.$fxLogger.createInfo('store-manage.in-store-order.fastOutStore', { selectRows, orderData: this.organDetails })
					return this.$fxApi('inStore.fastOutStore')({ data, logger }).then(res => {
