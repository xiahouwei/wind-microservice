...
export default {
	...
	methods: {
		onSearchClick () {
			if (this.filter.otherOrganId.length) {
		...
		async getAccountingPeriod () {
			...
			return this.$fxApi('inStore.getFilterAccountingPeriod').then(res => {
				...
				this.filter.otherOrganId = currentOrgan.id ? [currentOrgan.id] : [this.organIdList[0].listData[0].id]
		...
		getSelectData () {
			this.$fxApi('inStore.getOtherOrganSelect').then(res => {
