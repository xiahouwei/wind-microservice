		...
		return {
			...
			tableColumn: [
				...
				{ prop: 'item.spec', label: '规格', width: '120px' },
				{
					...
					label: '单位',
					editColumn: (row) => {
						return (!this.commonLimited || row.preDetailIsAssistUnit)
					},
				...
				{ prop: 'item.assistUnit', label: '辅助单位', width: '120px' },
				{
					...
					label: '辅助数量',
					editColumn: (row) => {
						return row.item && row.item.assistUnit && !this.commonLimited
					},
	...
	computed: {
		...
		tableEditColumnProp () {
			if (this.isAssign && this.systemParamMoneyEdit) {
				return ['amount', 'inTaxPrice', 'inTaxMoney', 'taxRate', 'inPrice', 'inMoney', 'assistUnitAmount']
			} else if (this.isAssign && !this.systemParamMoneyEdit) {
				return ['amount', 'assistUnitAmount']
			} else if (!this.haveCostPower && !this.isSaleOrderSubOutOrder && !this.isStockOrderSubOutOrder) {
				return ['unit', 'amount', 'assistUnitAmount']
			} else if (this.isSaleOrderSubOutOrder) {
				return ['storeCode', 'amount']
		...
		commonLimited () {
			return this.isSaleOrderSubOutOrder || this.isStockOrderSubOutOrder || this.isAssign
		},
		...
		showDeleteBtn () {
			return !(this.isSaleOrderSubOutOrder || this.isStockOrderSubOutOrder)
		},
		mistakeStepType () {
