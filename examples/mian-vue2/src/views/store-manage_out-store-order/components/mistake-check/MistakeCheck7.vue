<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1400px" heightFullscreen @close="clear">
		<div class="create-organ">
			<fx-mistake-check-step :activeStep="4" :type="mistakeStepType"></fx-mistake-check-step>
			<div class="mistake-check-warning fx-warn-font">注：以下明细中，存在出库品项为负库存出库，请再次确认！</div>
			<fx-details-table
				ref="detailsTable"
...
export default {
	...
	methods: {
		...
		onSaveClick () {
			this.onSaveTableListCheck().then(() => {
				this.dialogVisible = false
				this.$emit('on-finish', 'StockFkc')
			})
		},
		onDetailsRefresh () {
			this.$fxApi('outStore.getStockFkc', this.organDetails.id).then(res => {
