<template>
	<w-dialog v-model="dialogVisible" title="异常警告" width="1400px" heightFullscreen @close="clear">
		<div class="create-organ">
			...
			<div class="mistake-check-warning fx-warn-font">注：以下明细中，存在出库品项未配货，请再次确认！</div>
			<fx-details-table
				ref="detailsTable"
...
export default {
	...
	computed: {
		...
		mistakeActiveStep () {
			return this.systemParamAmountNegative ? 5 : 4
		},
		mistakeStepType () {
	...
	methods: {
		...
		onSaveClick () {
			this.onSaveTableListCheck().then(() => {
				this.dialogVisible = false
				this.$emit('on-finish', 'DistributionMarkNull')
			})
		},
		onDetailsRefresh () {
			this.$fxApi('outStore.getDistributionMarkNull', this.organDetails.id).then(res => {
