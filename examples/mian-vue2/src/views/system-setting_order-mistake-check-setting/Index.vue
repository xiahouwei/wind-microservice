<template>
	<w-app-container>
		<fx-page-header title="单据异常警告设置"></fx-page-header>
		<w-app-container direction="column" class="fx-page-first-container">
			<el-tabs
				v-model="currentTag"
				type="card"
				class="mistake-check-setting__tabs"
			...
			>
				<el-tab-pane
					...
					class="mistake-check-setting__pane"
				>
...
import OrderMistakeCheckSetting from './components/OrderMistakeCheckSetting.vue'
export default {
	...
	methods: {
		...
		tabContextRender (h, item) {
			...
			if (item.name === 'manage') {
				...
				return (
					<OrderMistakeCheckSetting options={item.options}></OrderMistakeCheckSetting>
				)
			}
		},
