<template>
	<w-app-container>
		<fx-page-header title="单据必填设置"></fx-page-header>
		<w-app-container direction="column" class="fx-page-first-container">
			<el-tabs
				v-model="currentTag"
				type="card"
				class="required-setting__tabs"
...
import OrderRequiredSetting from './components/OrderRequiredSetting.vue'
export default {
	...
	methods: {
		...
		tabContextRender (h, item) {
			...
			if (item.name === 'manage') {
				...
				return (
					<OrderRequiredSetting options={item.options}></OrderRequiredSetting>
				)
			}
		},
