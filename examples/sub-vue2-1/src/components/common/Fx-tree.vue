				...
				>
					...
					>
						...
						<el-tooltip :content="tipFormatter?tipFormatter(data):data[defaultProps.label]" placement="top-start" offset="-100" :enterable="false">
							...
							>{{ labelFormatter?labelFormatter(data):data[defaultProps.label] }}</span>
						</el-tooltip>
...
export default {
	...
	computed: {
		...
		isSearchFilterTreeNode (data, node) {
			return (data, node) => {
				return this.treeSearchKey && this.$fxUtils.fuzzyQueryObj(data, this.treeSearchKey, this.searchProps) && node.visible
			}
		},
		searchFilterTreeNodeList () {
			...
			return treeDataList.filter(item => {
				if (this.$refs.organTree) {
					...
					return this.treeSearchKey && this.$fxUtils.fuzzyQueryObj(item, this.treeSearchKey, this.searchProps) && node.visible
				}
