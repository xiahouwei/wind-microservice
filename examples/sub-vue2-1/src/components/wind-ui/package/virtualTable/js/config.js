...
const cellRender = {
	...
	expand: {
		...
		renderCell (h, column, columnIndex, rowItem, rowIndex, store) {
			...
			const isRowExpandHover = store.isRowExpandHover(rowItem)
			if (isExpanded) {
			...
			const mouseover = (e) => {
				store.commit('rowExpandHoverChange', rowItem.$fxIndex)
			}
			const mouseout = (e) => {
				store.commit('rowExpandHoverChange', null)
			}
			...
			return (<div class={ classes } on-click={callback} on-mouseover={mouseover} on-mouseout={mouseout}>
				{ isRowExpandHover ? <i class={['el-icon', arrowClass]}></i> : <span>{cell}</span>}
			</div>)
		}
	},
	arrow: {
