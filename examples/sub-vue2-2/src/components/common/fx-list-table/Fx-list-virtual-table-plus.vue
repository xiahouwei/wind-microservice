	...
	<div class="organ-list" :style="listStyle">
		...
		<div class="organ-list__container">
			...
			>
				...
				<w-virtual-table-column-plus v-if="expandRows.length>0" :fixed="selectAndIndexFixed" type="expand" :index="computedIndex" label="序号">
				</w-virtual-table-column-plus>
				<w-virtual-table-column-plus v-else-if="showIndex" :fixed="selectAndIndexFixed" type="index" :index="computedIndex" label="序号">
				</w-virtual-table-column-plus>
				<template v-for="item in currentTableColumn">
					<fx-list-virtual-table-column-plus
						v-if="item.visible"
						:key="item.prop"
						:item="item"
					></fx-list-virtual-table-column-plus>
		...
		<slot v-if="showSummaryBottom" name='summary-bottom'></slot>
	</div>
</template>
<script>
/**
...
export default {
	...
	data () {
		return {
			...
			selectRowCompareUtil: this.$fxUtils.compareArrayUtils(),
	...
	computed: {
		...
		columnFilterArr () {
			const currentSubTableColumn = this.currentExpandRows.filter(item => {
				return item.showFilter === true
			})
			return [
				...this.currentTableColumn,
				...currentSubTableColumn
			]
		},
		summaryColumns () {
	...
	watch: {
		...
		listData: {
			immediate: true,
			handler (listData, oldVal) {
				if (this.selectable) {
					if (listData !== oldVal) {
						this.updateSelectedRowArr(listData)
					}
					this.$nextTick(() => {
						this.renderSelect()
	...
	methods: {
		...
		onTableSubSelect (selectedMap, row) {
			this.subSelectedRowMap = selectedMap
		},
		...
		getSelectedRows (validate = true) {
			return new Promise((resolve, reject) => {
				let subEmpty = true
				for (const subSelect in this.subSelectedRowMap) {
					if (this.subSelectedRowMap[subSelect].length > 0) {
						subEmpty = false
					}
				}
				if (this.selectedRowArr.length === 0 && validate && subEmpty) {
					const error = '请选择一条明细'
		...
		getSingleSelectedRow () {
			return new Promise(resolve => {
				if (this.selectedRowArr.length === 0) {
					this.$fxMessage.warning('请勾选成品添加附成品或配料！')
				} else if (this.selectedRowArr.length > 1) {
					this.$fxMessage.warning('只能选择一条明细')
				} else if (this.selectedRowArr[0].additionFlag) {
					this.$fxMessage.warning('请勾选成品添加附成品或配料！')
				} else {
					const singleRow = this.listData.find(item => {
						return item.id === this.selectedRowArr[0].id
					})
		...
		updateSelectedRowArr (listData) {
			if (this.selectedRowArr.length) {
				this.selectRowCompareUtil.setData(listData, {
					key: 'id',
					value (item, index) {
						return index
					}
				})
				for (let i = 0; i < this.selectedRowArr.length; i++) {
					const id = this.selectedRowArr[i].id
					if (this.selectRowCompareUtil.has(id)) {
						const index = this.selectRowCompareUtil.get(id)
						this.selectedRowArr.splice(i, 1, listData[index])
					}
				}
			}
		},
		...
		getSubSelectedIds () {
			...
			this.listData.forEach(item => {
				...
				for (const dosing of item.dosingDetails) {
					if (dosing.selected) {
						detailIds.push(dosing.id)
						detailFlag = true
					}
				}
				if (detailFlag) {
					dosingDetailIdMap[item.id] = detailIds
				}
			})
			return dosingDetailIdMap
		},
		getAdditionProductDetailIdMap (list) {
			const additionProductDetailIdMap = {}
			list.forEach(item => {
				if (item.additionFlag) {
					if (Object.prototype.hasOwnProperty.call(additionProductDetailIdMap, item.parentId)) {
						additionProductDetailIdMap[item.parentId].push(item.id)
					} else {
						additionProductDetailIdMap[item.parentId] = [item.id]
					}
				}
			})
			return additionProductDetailIdMap
		},
		clearSelected () {
			this.selectedArr = []
			this.selectedRowArr = []
		...
		clearSubSelection () {
			this.subSelectedRowMap = {}
			this.$refs.organTable.clearSubSelection()
		},
		clearCurrentSelection (rowId) {
			this.$refs.organTable.clearCurrentSelection(rowId)
		},
		...
		onCellSubClick (subRow, exrow, row) {
			if (typeof exrow.click === 'function') {
				exrow.click(subRow, row)
			}
		},
		...
		async verificationSubRowData (row, mainRowIndex) {
			for (let i = 0; i < this.subRowValiDate.length; i++) {
				try {
					await this.subValiDate(row, this.subRowValiDate[i], mainRowIndex)
				} catch (error) {
					return Promise.reject(error)
				}
			}
		...
		subValiDate (subRow, subValiDateItem, mainRowIndex) {
			return new Promise((resolve, reject) => {
				const success = () => {
					const validate = {
						columnProp: subValiDateItem.columnProp,
						index: subRow.$fxIndex,
						type: 'success'
					}
					this.tableEventBus.$emit('$fx-table-row-validate', validate)
					resolve()
				...
				const error = (message) => {
					const validate = {
						columnProp: subValiDateItem.columnProp,
						index: mainRowIndex,
						subIndex: subRow.$fxIndex,
						type: 'error',
						message
					}
					if (this.isRenderedRow(subRow.$fxIndex) && this.$refs.organTable.isRowInView(subRow.$fxIndex)) {
						this.tableEventBus.$emit('$fx-table-row-validate', validate)
						reject(message)
					} else {
						this.scrollToIndex(subRow.$fxIndex).then(() => {
							this.tableEventBus.$emit('$fx-table-row-validate', validate)
							reject(message)
						})
					}
				}
				subValiDateItem.rule(subRow, success, error)
			})
		},
		...
		cellSelectedHandler (cellEvent) {
			if (this.tableEditColumnProp.length > 0) {
				...
				if (typeIndex !== this.tableEditColumnProp.length - 1 && typeIndex !== -1) {
					const nextType = this.tableEditColumnProp[typeIndex + 1]
					this.setListCellFocus(this.createCellEvent(nextType, cellEvent))
				}
			}
		...
		setAllExpand () {
			...
			this.rowExpanded = this.expandRowKeys
		},
		clearExpanded () {
			this.expandRowKeys = []
			this.rowExpanded = []
		...
		// 新增原料展开
		setCurrentExpand () {
			this.getSingleSelectedRow().then(row => {
				this.expandRowKeys = [row.id]
				this.rowExpanded = [row.id]
			})
		},
		// 切换bom展开
		...
		setBomAddExpand (row) {
			this.$nextTick(() => {
				...
				this.expandRowKeys = this.rowExpanded
			})
		},
