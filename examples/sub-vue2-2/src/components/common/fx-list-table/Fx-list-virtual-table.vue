	...
	<div class="organ-list" :style="listStyle">
		...
		<div class="organ-list__container">
			<w-virtual-table
				...
				@select-collection="onTableRowCollectionSelect"
 ...
 * @param {Array} extraSummaryColumns - 额外的合计列
...
export default {
	...
	computed: {
		...
		summaryColumns () {
			return [...this.tableColumn.filter(item => !!item.summary), ...this.extraSummaryColumns]
		},
		selectSummaryData () {
	...
	methods: {
		...
		onTableRowCollectionSelect (selecteds, selectFlag, collectionIds) {
			const selectType = 'collection'
			if (!selectFlag) {
				const collectionIdsCompareArrayUtils = this.$fxUtils.compareArrayUtils(collectionIds)
				this.selectedRowArr = this.selectedRowArr.filter(item => {
					return !collectionIdsCompareArrayUtils.has(item.id)
				})
			} else {
				selecteds.forEach(item => {
					if (!this.compareArrayUtils.has(item.id)) {
		...
		setListCellFocus (cellEvent, immediate = true) {
			...
			if (immediate && this.$refs.organTable.isRowInView(index)) {
		...
		renderSelect () {
			...
			if (this.selectMulity) {
				...
				if (this.selectedRowArr[0]) {
					this.$refs.organTable.radioSelection(this.selectedRowArr[0])
				}
			}
		},
