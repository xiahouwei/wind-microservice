<template>
	<w-dialog v-model="dialogVisible" width="1200px" title="关联品项" heightFullscreen @close="close">
		...
		<template slot="footer">
			...
			<el-button
				v-fx-auth:basic-archives_support-archives_take-stock-class.edit-item-custom-group
				type="primary"
				@click="onSaveClick"
			>保存</el-button>
		</template>
...
import { apiName } from '../ItemCustomGroupCreateDialog.vue'
	...
	methods: {
		...
		onSaveClick () {
			...
			return new Promise(resolve => {
				...
				const logger = this.$fxLogger.createInfo('basic-archives.support-archives.item-custom-group.connectItem', this.createLoggerParams(this.fxDataVerification.getOriginData(), selectRows))
				this.$fxApi(`${apiName}.settingItem`, this.itemDetails.id)({ data: this.listSelectIds, logger }).then(() => {
					this.fxDataVerification.resetData(selectRows)
					resolve()
				})
			})
